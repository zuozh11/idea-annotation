...
IntelliJ Platform 当前哪些能力能取得编辑器选区与源文件、提供原生批注入口和输入界面、控制浮动工具栏并写入系统剪贴板；本插件应采用什么 Gradle、JDK、IDE 构建号与最低兼容版本？
...
- **浮动工具栏控制**：IDEA 2026.2 的 `CodeFloatingToolbar.getToolbar(editor).scheduleHide()` 能在 Action 触发后隐藏当前工具栏；该能力属于当前平台实现，本插件允许在固定的 `262` 目标范围内直接使用，以获得与 IDEA“询问 AI”一致的交互。
...
结论：主体能力使用公开 API；为贴合 IDEA 2026.2 原生交互，可以直接使用当前平台实现提供的浮动工具栏控制能力，不再把“禁止内部 API”作为项目锁定条件。
