...
- 先删除每个选区首部所有空白行（空字符串或只包含空格/Tab）。剩余内容中的空白行不参与公共前缀计算，输出时统一为空行；内部和尾部空白行保留行边界。如果任一非空行顶格，则公共前缀为空，不改变缩进。
...
| 选区缩进 | 每个选区先删除首部空白行，再移除非空行共有的最长前导空白前缀，保留相对缩进及内部和尾部换行结构 | [选区整体缩进移除](./wayfinder/16-选区整体缩进移除.md) |
